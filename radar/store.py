...
def upsert(conn: sqlite3.Connection, r: Release) -> None:
    conn.execute(
        """
        INSERT INTO releases (dedup_key, company, product, title, summary, date, type, url, tags)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        (
            ...
            r.date, r.type.value, r.url, json.dumps(r.tags),
        ),
    )
    conn.commit()
...
def load_all(conn: sqlite3.Connection) -> list[dict]:
    ...
    rows = conn.execute(
        "SELECT company, product, title, summary, date, type, url, tags "
